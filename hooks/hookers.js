module.exports = function (ctx) {
  const Q = ctx.requireCordovaModule('q'),
    ...
    staticFolder = path.resolve(pRoot, './static/'),
  ...
  const sys = {
    ...
    makeDevServerChanges() {
      let defer = new Q.defer(),
        configFile = path.resolve(__dirname, '../config.xml'),
        srcFile = path.resolve(__dirname, '../build/cordova-dev/device_router.html'),
    ...
    startWebpackDevServer() {
      let defer = new Q.defer(),
        outText = '',
        isResultFound = false,
        args = [`"${webpackDevServerPath}"`,'--inline', '--env.devserver', '--config build/webpack.dev.conf.js',`--public ${getRouterIpAddr()}:8081`],
        run = epipeBombPath

      if( os.platform() === 'win32' ) {
        args = ['--inline', '--env.devserver', '--config build/webpack.dev.conf.js',`--public ${getRouterIpAddr()}:8081`]
    ...
    addUpdate(p) {
      let rel = path.relative('./static/', p)
    ...
    cplatform_cordova_filesopyFile(source, target, cb) {
      let cbCalled = false
